...
对象终结器抛错时仍完成引用释放，并保留原有执行或构造异常。深层 Array/Dictionary 链通过有界释放队列清理；显式失效重试、对象保留自身、断环、暂停/取消和清理分配失败已有专项验证。TJS2 使用引用计数，任意循环引用仍需由脚本显式断开；宿主对象的隐式资源回收仍在实现。范围见 [对象终结](docs/decisions/038-object-finalization.md)。
