...
6. Bytecode compilation mode belongs to `tTJSScriptBlock`, so nested compilation from a console observer cannot clear the outer block's mode. Member export metadata in `tjsInterCodeGen.cpp/h` uses a value-owned vector instead of allocations released only on successful export; aborted compilation and ordinary nested evaluation have independent lifetimes. The hosted Scripts fixtures cover successful and failed nested compilation, class constructors/methods/accessors, and loading the output in a fresh VM.
7. Bison syntax errors enter `_yyerror` and failed parses cannot proceed to execution/export. `tjsByteCodeLoader.cpp` accepts the exporter’s `-1` sentinel for unnamed contexts and allocates debug tables with `TJS_malloc`, matching their `TJS_free` cleanup. Native bytecode character offsets remain unchanged; without a serialized source line table, loaded bytecode still reports line 1.

8. Source preparation, lexer/parser work, code generation and bytecode export have cooperative checkpoints. Temporary octet literals and export buffers use scoped ownership, and export writes are chunked. These changes share the existing VM continuation and cancellation control; they do not preempt individual native allocations or library calls.
9. `tjsByteCodeLoader.cpp` runs the project-owned bounded structural validator before materializing a file, uses bounded string construction and reuses native string references. Integer decoding uses unsigned shifts, and VM dispatch guards its instruction pointer before dereferencing it. The transactional loader changes below cover construction ownership; deep call/try resource limits remain under review.
10. `tjsBinarySerializer.cpp/h` validates KBAD values before construction, bounds bytes/counts/depth, cleans up temporary values on errors and checks cooperative cancellation. Negative fix integers are signed; native NUL-terminated strings and nonempty dictionary names remain unchanged. Scripts and startup resource loading reuse this serializer through the native bridge. Details and hosted verification history are recorded in `docs/decisions/034-binary-scripts.md`.

11. Bytecode pools and context buffers have scope ownership; successful linking drops initial construction references, and interrupted linking invalidates the unpublished graph before release. Context registration is delayed until construction succeeds, and removal uses a stored list iterator. The validator bounds expanded names/symbols and rejects orphan or cyclic ownership. Shared string heap growth and compiler buffer replacement preserve old allocations until new allocation succeeds. Optional read-only allocation/context counters and a separate non-release allocator diagnostic support hosted lifecycle verification; see `docs/decisions/036-bytecode-lifetime.md`.
