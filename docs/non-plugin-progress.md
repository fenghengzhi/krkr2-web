...
首轮 Node 的 2 个预期失败和首次完整回归的 19 个浏览器失败继续保留。18 个浏览器图形夹具已明确初始类型／颜色；PNG 停止用例的点击晚于编码完成，后续绿色重跑不能证明这一时序已修复。阶段 050 的受控取消测试仍在独立分支。

上述阶段 046 结果对应本版本。阶段 047 Window.mainWindow 已实现但[完整回归 34930203580](https://github.com/fenghengzhi/krkr2-web/actions/runs/34930203580)的浏览器检查为 821/822 通过、1 项 WebKit JSPI 启动用例 Page crashed，仍待诊断与完整验证；阶段 049 空矩形 piledCopy 和阶段 050 取消测试也未纳入此版本。当前仍只允许一个活动 Window，完整多窗口、其余图形／系统 API、流式媒体、旧视频编码与菜单嵌套事件等仍未完成，整体非插件目标继续进行。
...
| Layer/Bitmap：图像坐标、排序、裁剪、像素/命中、混合、转场           | 已补 26 种像素混合、矩形/缩放/仿射、类型转换/灰度/模糊、整图翻转、Gamma、组透明度、转场、onPaint、截图和 BMP；046 另验证可写 neutralColor、无主图 opaque 填色和 piledCopy 主图前置检查；空矩形 piledCopy 的 049 分支、其他像素方法与完整几何/转场差分仍未完成 | 进行中 |
...
| 验证：参考 KAG 对话/选择/转场/声音/脚本存读档、三浏览器、差分与性能 | 046 云端 1,043 项 Node、786 项浏览器与 6 组直接运行时通过；相同应用源码的 78 项原 KAG／离线升级通过。既往分配故障、冷重启、原生日志、字体和像素证据按历史阶段保留；047 页面崩溃、050 PNG 取消时序、完整原生差分和性能仍待验证                                 | 未完成 |
