...
| Timer/AsyncTrigger/事件、Window、完整输入与系统 API                 | 已补窗口生命周期与 mainWindow 查询、焦点/模态、鼠标/触摸捕获、脚本命中、键盘/提交文字和异步输入；System 事件/连续回调、异常处理与菜单门控已接入；051 页面内多窗口已验证；Window.showModal、菜单嵌套、手势/完整 IME、原生全屏和全部事件重入仍需补齐  | 未完成 |
...
| 验证：参考 KAG 对话/选择/转场/声音/脚本存读档、三浏览器、差分与性能 | 051 版本 1,317 项 Node、1,041 项浏览器与 6 组直接运行时通过；同次构建的 78 项原 KAG／离线升级通过。050 取消与 051 多窗口证据已计入总数；既往失败按阶段保留，历史 V8/WebKit 故障归因、真实用户控制延迟、完整原生差分与性能仍待验证                   | 未完成 |
