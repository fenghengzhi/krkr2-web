# 036 — 字节码加载的所有权与失败回滚

字节码加载按验证、常量池、上下文构造、链接四步进行。加载器保留每个新对象的初始引用，直到所有链接安装完成；成功后释放这些初始引用，失败或取消时先失效所有尚未发布的上下文，再释放引用。上下文只在构造成功后登记到脚本块，并用保存的列表位置移除，避免大量上下文回收时反复扫描列表。

常量池、代码、变体、调试位置和名字使用作用域所有权。字符串池扩容先分配完整的新存储，再替换旧索引。源码构造路径共用的缓冲区扩容也只在分配成功后替换指针。超类 getter 和属性 getter/setter 持有一致的引用。

验证器拒绝从顶层不可达的上下文和强引用环，并按常量、名字、上下文及符号记录收取 128 MiB 的构造预算。这个保守预算用于限制小输入展开成大量原生结构的情况，不代表任意时刻堆占用的精确上界，也不代替分配失败处理。

阶段 6、7、8 分别在常量池、上下文构造和链接循环中使用现有的时间片检查。暂停时保留局部所有权，取消时正常展开原生异常并清理。只读诊断出口报告原生分配字节、字符串单元、脚本块和上下文数量；WASM 线性内存长度不用于证明活跃分配已释放。[Emscripten 内存诊断文档](https://emscripten.org/docs/porting/Debugging.html) 描述了 `mallinfo()` 的用途。

独立分配故障构建由 `KRKR_TEST_ALLOCATOR=1` 启用，使用独立构建目录和源身份；清单明确标为诊断内核。正式构建默认关闭，不能将诊断内核当作发布验证证据。

验证仅在 GitHub-hosted Actions 运行。本轮的新生命周期用例和故障诊断尚未取得完成结果；旧二进制脚本基线仍保存在决策 034、035 和历史报告中。VM 调用/try 栈预算、其他原生分配路径和历史 KAG 异常仍是后续工作，不能据此声明非插件部分全部完成。

首轮 `34846307441` 的 Node 结果为 388/391，取消三个阶段均通过，但继承文件被拒绝，两条恢复用例保留了方法上下文。原生编译器为 superclass proxy 写入 `RET; NOP` 尾部，验证器此前将这个哨兵误认为可执行 fallthrough；现在只允许该上下文的这个特定尾部，并禁止跳转到哨兵。当时将保留现象归因于实例自身闭包，并在生命周期用例中显式 `invalidate` 实例再删除全局绑定；这是当时的假设，并非已验证根因。后续 [决策 037](037-execution-budgets.md) 发现对象存储已有自身闭包引用调整，修正退出帧残留寄存器，并加入不显式 invalidate 的自动终结用例。任意对象环的回收仍是独立未完成项；本阶段的显式清理结果及原失败不变。

首轮故障工作流 `34846470235` 的两个内核均在链接阶段失败，未执行任何故障用例：`--wrap=malloc` 改变了 Emscripten JavaScript glue 要求的导出。诊断改用工具链自带 `test/core/test_wrap_malloc.c` 所使用的 `emscripten_builtin_malloc/calloc/realloc` 接口；正式内核仍不含故障注入。

第二次诊断 `34847308669` 中 Asyncify 已通过 92 次逐点分配失败（池 24、上下文 67、链接 1）与字符串堆块/索引两次定向失败，销毁后的分配字节和字符串单元与无故障对照相同。JSPI 在模块创建前失败：固定 Node 24.19.0 默认没有启用 `WebAssembly.Suspending`。工作流现在明确传入 `--experimental-wasm-jspi`；旧运行仍是整体失败，不能计为双后端通过。

[双后端分配诊断 34847772015](https://github.com/fenghengzhi/krkr2-web/actions/runs/34847772015) 已通过：每个后端覆盖池 24、上下文 67、链接 1 次逐点失败及字符串堆块/索引各 1 次定向失败，总共 **188 次故障注入**。逐点检查都恢复到预热后的原生字节、字符串单元及零脚本块/上下文，再用同一个 VM 重载完整继承/访问器脚本。两种后端的堆扩容无故障对照和两类失败后的销毁记录均为 571,992 分配字节、81 个模块静态字符串单元；这些静态单元属于仍存活的模块，不要求其数量为零。不能将本夹具的枚举覆盖理解为所有原生路径或任意内存耗尽均已验证。

完整运行 `34847758637` 的 392 项 Node 已通过，直接运行时在 Chromium JSPI 的阶段 6 用例失败：单个 8 MiB 字符串的复制在时间片内完成，没有触发暂停检查。用例改为固定 32,700 个独立的 256 单元字符串，覆盖实际分配和字符串驻留工作，再在已分配临时资源的检查点暂停；没有放宽暂停、取消或回收断言。运行时探测现在收齐六个组合的失败/成功结果，任何失败仍使工作流失败，不使用重试。

[更新夹具后的运行时专项 34848864432](https://github.com/fenghengzhi/krkr2-web/actions/runs/34848864432) 已通过全部六个三浏览器/双后端组合；[同版夹具分配诊断 34848868196](https://github.com/fenghengzhi/krkr2-web/actions/runs/34848868196) 也已通过。正式应用源码与 [78 项兼容专项 34848253401](https://github.com/fenghengzhi/krkr2-web/actions/runs/34848253401) 完全一致；KAG 不引用该字节码生命周期夹具，报告将其版本分别绑定到完整回归和分配诊断。

运行 `34847758637` 的完整浏览器结果为 638/639：WebKit JSPI 的原有 `native Debug classes and dump files survive a cold offline browser restart` 在冷启动后报告 `Page crashed`（约 5.35 秒），不是等待断言超时。原 JSON、截图、DOM/网络 trace 和持久上下文记录已归档；新增固定 20 次该原场景的 macOS 专项，收集 Playwright 浏览器进程日志与操作系统崩溃报告。该页面崩溃原因尚未确认，不能视为已由字节码所有权修复解释。

最终 [Tests 34849454871](https://github.com/fenghengzhi/krkr2-web/actions/runs/34849454871) 的全部 14 个作业已通过；此版包含 392 项 Node、639 项浏览器及 6 项直接运行时。[WebKit 离线崩溃专项 34849908821](https://github.com/fenghengzhi/krkr2-web/actions/runs/34849908821) 的 20 个原场景均通过，没有重试、跳过或新 macOS 崩溃报告，仍未确认原页面崩溃的根因。完整报告将绑定该回归、78 项兼容性、188 次分配失败及这 20 次独立重启。

## 最终绑定

[报告 34850540242](https://github.com/fenghengzhi/krkr2-web/actions/runs/34850540242) 已通过，包含 554 份证据，矩阵 `out/verification/bytecode-lifetime-matrix.json` 的 SHA-256 为 `c3c5c665b1de52cc989edc0a3abad39001c0db1fc560baa49b1dfe63f798089b`。报告绑定 Tests `34849454871`、兼容专项 `34848253401`、分配诊断 `34848868196` 与冷重启专项 `34849908821`，所选案例无失败、跳过、flaky 或重试。原失败及额外的运行时专项 `34848864432` 单独保留。

正式 build token 为 `7bfdc47d6e13a4967ed450c7f8d47643b2c61493a0995dd305f99ab8cc4edea7`；TJS 源身份为 `7e1e54bfd0a8dc8fe5cee470c385a133dacc692dc442ca0f0510aa774d28b6df`，`diagnosticAllocator: false`。`src` 为 160 文件、900,619 字节，树摘要 `d7a200d32b6b56d57e255035f4cfd2c693b48a65f509d6460a1e1d10b708a542`；正式发布为 32 文件、5,706,478 字节，树摘要 `a892918a0af501e6d2e4b4ad9a893b44bb264064eb3064e70ad0e175fd1dae51`。可信冻结 21,059.7 ms；字体 ABI 2、协议 9 保持不变。

本地 `.generated` 和 `dist` 仅从该 Tests 的精确 `test-build` artifact 恢复，没有执行本地测试或探测；原本地二进制脚本产物保存于 `out/verification/bytecode-lifetime/prior-local-artifacts/`。最终文档更新发生在报告生成后，不改变应用和测试实现。完整非插件目标继续保持进行中。
