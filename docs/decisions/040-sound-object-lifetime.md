...
已实现并验证声音弱观察、事件持有、异步关闭队列、flags 原生失效、labels 延后失效和后端迟到解码隔离。本阶段覆盖下述所有权范围；完整 MIDI、CD 映射、流式媒体与其余非插件目标仍未完成。本文件保留实现前审计和全部失败记录。所有构建、测试和可执行探测只在 GitHub-hosted Actions 运行。

## 最终验证

[完整回归 34895849611](https://github.com/fenghengzhi/krkr2-web/actions/runs/34895849611) 通过 710 项 Node、651 项浏览器和 6 组直接运行时；[兼容性 34894024462](https://github.com/fenghengzhi/krkr2-web/actions/runs/34894024462) 通过 78 项。直接运行时包含 60 个真实声音、168 个从属对象场景，独立诊断另有 64 个宿主句柄和 120 个对象用例。分配诊断通过 600 次观察/升级/销毁、24 次从属对象、20 次集合清理、1,061 次执行与 188 次字节码分配失败。

[最终报告 34897329847](https://github.com/fenghengzhi/krkr2-web/actions/runs/34897329847) 在 `fb113cfce0166b625bfb1770751fbf23c13b7376` 上核对同源构建和测试，绑定 542 份证据。矩阵 `out/verification/sound-object-lifetime-matrix.json` 的 SHA-256 为 `bd7c8848e1e0183457644d67a8f6dbfbf41a8a951576db2f608046e7e60307da`；本轮可信冻结 21,053.1 ms，没有额外重复冻结或字体重启诊断计入本阶段结果。

应用源码 161 个文件、918,399 字节，树哈希 `d80fe812d4bb8693a2d1167b1c3789a0e18feb562443c6e2dcc969fbcafb35b2`。发布 32 个文件、5,775,800 字节，树哈希 `28e04739ade014aa3c08431fa0573bd1bcb5442c4baed33efb53c37b9e45ecec`，构建 ID `387f9483b2e410d6e8da65db197bfc39b1d9593c63915996407a42841cebaac0`。TJS 内核源哈希为 `2890036ed3eea4a63d83dc623c98030c64be0e856ef2bf7628558a26bc3130db`，TJS ABI 5、字体 ABI 2、协议 9 不变，新增能力 `soundObjectLifetime: 1`。本地仅备份旧产物并恢复该云端构建，未执行本地验证。

首份证据报告 [34897104201](https://github.com/fenghengzhi/krkr2-web/actions/runs/34897104201) 因每浏览器常规套件仍预期 172 项而失败；新增四个声音场景后实际为 176 项。修正报告计数后生成上述最终报告。此失败报告、两次完整回归失败及早期通过运行均按 run ID 归档，没有删除或改记为通过。下文的“进度”和“原始审计”按历史记录保留，当前结论以本节为准。
...
该参数夹具现仅在自身执行期间临时推进 `performance.now`，在真正观察到 phase-10 的原生临时内存与堆增长后停止推进，并在 finally 恢复原始属性。Emscripten 产物确实从此时钟读取 deadline。真实 25 ms 暂停等待、恢复/取消结果、上下文与堆回收断言保留，报告额外记录时钟模式与读取次数。产品代码和 8 ms 时间片没有改变；这项可控检查不构成物理最坏延迟证明。修正后的 [运行时专项 34895391871](https://github.com/fenghengzhi/krkr2-web/actions/runs/34895391871) 六组通过；同一测试源码的 [分配诊断 34895417115](https://github.com/fenghengzhi/krkr2-web/actions/runs/34895417115)、[对象诊断 34895421775](https://github.com/fenghengzhi/krkr2-web/actions/runs/34895421775) 与 [句柄诊断 34895425697](https://github.com/fenghengzhi/krkr2-web/actions/runs/34895425697) 也通过，完整回归和最终报告尚待完成。原始 34894629812 最终为 Node 710/710、浏览器 651/651、直接运行时 4/6，整个运行保留为失败。
