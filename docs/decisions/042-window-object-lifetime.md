...
以已验证的视频阶段 `2cc4ab4` 为基线，在 `codex/window-lifetime` 独立分支实现。全部构建、类型检查和测试只在 GitHub-hosted Actions 运行。选定的窗口生命周期现已通过完整回归与最终证据报告；完整多窗口、关联对象和其余非插件功能仍未完成。

[34914435535](https://github.com/fenghengzhi/krkr2-web/actions/runs/34914435535) 的 14 个作业全部通过，包含 **900 Node、678 浏览器检查、6 直接运行时**；另有 **78 项 KAG／离线升级、64 项隔离句柄和 120 项隔离对象检查**通过。[最终报告 34916018555](https://github.com/fenghengzhi/krkr2-web/actions/runs/34916018555)已逐项核对当前应用、常规测试、构建和专项证据，绑定 543 份材料，矩阵 SHA-256 为 `e5a2da13b7838024e29c01b51c03c8629377d4a7d8aa6cb21e6eb59c7a435f5e`。报告生成于 `3219099`，后续说明不修改已验证应用代码。下文保留实施与失败历史。
