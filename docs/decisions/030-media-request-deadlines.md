...
新增虚拟时钟测试覆盖剩余期限、冻结中新建/取消请求及过期回调失效；真实 TJS 集成测试检查生命周期通知与媒体命令的顺序。验证只在 GitHub Actions 执行。修复后 346 项 Node、7 项原生生命周期检查已通过；[独立长冻结复测](https://github.com/fenghengzhi/krkr2-web/actions/runs/34809075784) 三次均通过，实际冻结分别约 21,061、21,059、21,053 毫秒，事件均为可信。[完整浏览器回归](https://github.com/fenghengzhi/krkr2-web/actions/runs/34809918318)也已通过全部 603 项测试。原生用例继续要求超过 21 秒的真实冻结、可信事件和 30 秒正文预算。
